...
const Pagos = () => {
  ...
  const [pagosPorPaciente, setPagosPorPaciente] = useState({});
  ...
  // Función para obtener pagos de un paciente
  const fetchPagos = async (cedula) => {
    setLoadingPagos(prev => ({ ...prev, [cedula]: true }));
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_PAGOS_BASE_URL}/ObtenerPagosxCedula/${cedula}`, {
        headers: {
          'Authorization': token ? `Bearer ${token}` : undefined,
          'Accept': 'application/json'
        }
      ...
      setLoadingPagos(prev => ({ ...prev, [cedula]: false }));
    }
  };

  // Manejar expansión de paciente para ver sus pagos
  const togglePacienteExpansion = (cedula) => {
    if (expandedPaciente === cedula) {
      setExpandedPaciente(null);
    } else {
      setExpandedPaciente(cedula);
      if (!pagosPorPaciente[cedula]) {
        fetchPagos(cedula);
      }
    }
  };
  ...
  // Calcular total de pagos por paciente
  const calcularTotalPagos = (cedula) => {
    if (!pagosPorPaciente[cedula] || pagosPorPaciente[cedula].length === 0) return 0;
    return pagosPorPaciente[cedula].reduce((total, pago) => total + (parseFloat(pago.monto) || 0, 0), 0);
  };
                        ...
                        <table className="min-w-full border mt-2">
                          <thead>
                            <tr className="bg-gray-50">
                              ...
                              <th className="border px-4 py-2 text-left">Total Pagos</th>
                            </tr>
                          </thead>
                          <tbody>
                              ...
                              <React.Fragment key={idx}>
                                <tr className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                  <td className="border px-4 py-2">
                                    <button
                                      onClick={() => togglePacienteExpansion(p.cedula)}
                                      className="p-1 rounded hover:bg-gray-200"
                                    >
                                      {expandedPaciente === p.cedula ? (
                                  ...
                                  <td className="border px-4 py-2">
                                    <div className="flex items-center">
                                      ...
                                      {calcularTotalPagos(p.cedula).toFixed(2)}
                                    </div>
                                  </td>
                                </tr>
                                {expandedPaciente === p.cedula && (
                                  <tr className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                    <td colSpan="6" className="border px-4 py-2">
                                      <div className="pl-8 pr-4 py-2">
                                        ...
                                        {loadingPagos[p.cedula] ? (
                                          <p>Cargando pagos...</p>
                                        ) : (
                                          pagosPorPaciente[p.cedula] && pagosPorPaciente[p.cedula].length > 0 ? (
                                            <table className="min-w-full bg-gray-50 rounded">
                                              ...
                                              <tbody>
                                                {pagosPorPaciente[p.cedula].map((pago, pagoIdx) => (
                                                  <tr key={pagoIdx}>
                                                    <td className="px-4 py-2">{pago.fecha || 'No especificada'}</td>
                                                    ...
                                                    <td className="px-4 py-2">{pago.descripcion || 'Sin descripción'}</td>
                                                    <td className="px-4 py-2">{pago.metodoPago || 'No especificado'}</td>
                                                  </tr>
                                                ))}
                                              </tbody>
                                            </table>
                                          ) : (
                                            <p className="text-gray-500">No se encontraron pagos para este paciente.</p>
                                          )
                                        )}
                                      </div>
                                    </td>
